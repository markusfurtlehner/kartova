...
public static class CommandLine
{
    ...
    public static int Run(string[] args)
    {
        ...
        {
            Console.Error.WriteLine($"dirstat: {e.Message}");
            return ExitFailure;
        }
    }
    ...
    private static int Dispatch(string[] args)
    {
        ...
        {
            Console.WriteLine($"DirStat {typeof(CommandLine).Assembly.GetName().Version?.ToString(3) ?? "1.0.0"}");
            return ExitSuccess;
        }
        ...
        {
            Console.Error.WriteLine("dirstat: --scan <path> is required. Try --help.");
            return ExitUsage;
        }

        if (!Directory.Exists(target))
        {
            Console.Error.WriteLine($"dirstat: not a directory: {target}");
            return ExitFailure;
        }
    ...
    {
        // --compare old.dirstat,new.dirstat  or  --compare old.dirstat --scan <path>
        var parts = (pair ?? string.Empty).Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
        if (parts.Length == 0)
        {
            Console.Error.WriteLine("dirstat: --compare needs a snapshot, and either a second snapshot or --scan.");
            return ExitUsage;
        }
        ...
        if (before is null)
        {
            Console.Error.WriteLine($"dirstat: could not read snapshot {parts[0]}");
            return ExitFailure;
        }
        ...
        if (parts.Length > 1)
        {
            ...
            if (second is null)
            {
                Console.Error.WriteLine($"dirstat: could not read snapshot {parts[1]}");
                return ExitFailure;
            }
            after = second.Root;
        }
        ...
        {
            Console.Error.WriteLine("dirstat: --compare needs a second snapshot or --scan <path>.");
            return ExitUsage;
        }
    ...
    {
        Console.WriteLine("""
            DirStat — disk usage analyser

            USAGE
              dirstat                                  open the window
              dirstat <path>                           open the window and scan a path
              dirstat --scan <path> [options]          scan without a window
            ...
            EXAMPLES
              dirstat --scan /home/me --duplicates
              dirstat --scan C:\\ --snapshot --quiet
              dirstat --compare monday.dirstat --scan C:\\
            """);
    }
...
public static class SnapshotStore
{
    public static string DefaultDirectory
    {
        get
        {
            ...
            return Path.Combine(root, "DirStat", "snapshots");
        }
    }
