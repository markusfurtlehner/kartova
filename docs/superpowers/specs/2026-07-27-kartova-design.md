...
on large volumes. Kartova keeps their capabilities and replaces the interaction and
rendering layer.
...
```
Kartova.Core        scanning, treemap layout, rasterization, exports, shell ops
Kartova.App         Avalonia views, view models, theming
Kartova.Core.Tests  xUnit
```
...
`Kartova.Core.Tests` covers the parts where correctness is not visually obvious:
